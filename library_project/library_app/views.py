...
def after_login_view(request):
    if is_admin(request.user):
        return render(request,'library_app/adminafterlogin.html')
    else:
        return render(request,'library_app/studentafterlogin.html')
...
def addbook_view(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            user=form.save()
            return render(request,'library_app/bookadded.html')
    return render(request,'library_app/addbook.html',{'form':form})

@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def viewbook_view(request):
    books=models.Book.objects.all()
    return render(request,'library_app/viewbook.html',{'books':books})
...
def issuebook_view(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            obj.save()
            return render(request,'library_app/bookissued.html')
    return render(request,'library_app/issuebook.html',{'form':form})
...
def viewissuedbook_view(request):
    ...
    return render(request,'library_app/viewissuedbook.html',{'li':li})

@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def viewstudent_view(request):
    students=models.StudentExtra.objects.all()
    return render(request,'library_app/viewstudent.html',{'students':students})
...
def aboutus_view(request):
    return render(request,'library_app/aboutus.html')
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            send_mail(str(name)+' || '+str(email),message, EMAIL_HOST_USER, ['nithiyakalyani202@gmail.com'], fail_silently = False)
            return render(request, 'library_app/contactussuccess.html')
    return render(request, 'library_app/contactus.html', {'form':sub})
